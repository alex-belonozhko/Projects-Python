# task#10

# В таблице 20×20 (внизу) четыре числа на одной диагонали выделены красным.
# 08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
# 49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00
# 81 49 31 73 55 79 14 29 93 71 40 67 53 88 30 03 49 13 36 65
# 52 70 95 23 04 60 11 42 69 24 68 56 01 32 56 71 37 02 36 91
# 22 31 16 71 51 67 63 89 41 92 36 54 22 40 40 28 66 33 13 80
# 24 47 32 60 99 03 45 02 44 75 33 53 78 36 84 20 35 17 12 50
# 32 98 81 28 64 23 67 10 26 38 40 67 59 54 70 66 18 38 64 70
# 67 26 20 68 02 62 12 20 95 63 94 39 63 08 40 91 66 49 94 21
# 24 55 58 05 66 73 99 26 97 17 78 78 96 83 14 88 34 89 63 72
# 21 36 23 09 75 00 76 44 20 45 35 14 00 61 33 97 34 31 33 95
# 78 17 53 28 22 75 31 67 15 94 03 80 04 62 16 14 09 53 56 92
# 16 39 05 42 96 35 31 47 55 58 88 24 00 17 54 24 36 29 85 57
# 86 56 00 48 35 71 89 07 05 44 44 37 44 60 21 58 51 54 17 58
# 19 80 81 68 05 94 47 69 28 73 92 13 86 52 17 77 04 89 55 40
# 04 52 08 83 97 35 99 16 07 97 57 32 16 26 26 79 33 27 98 66
# 88 36 68 87 57 62 20 72 03 46 33 67 46 55 12 32 63 93 53 69
# 04 42 16 73 38 25 39 11 24 94 72 18 08 46 29 32 40 62 76 36
# 20 69 36 41 72 30 23 88 34 62 99 69 82 67 59 85 74 04 36 16
# 20 73 35 29 78 31 90 01 74 31 49 71 48 86 81 16 23 57 05 54
# 01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48
# Произведение этих чисел 26 × 63 × 78 × 14 = 1788696.
# Каково наибольшее произведение четырех подряд идущих чисел в таблице 20×20,
# расположенных в любом направлении (вверх, вниз, вправо, влево или по диагонали)?

# Нашел макс число по горизонтали , по вертикали и диагонали так сделать не получеться

# OurNumber = ('0802229738150040007504050778521250779108'
#              '4949994017811857608717409843694804566200'
#              '8149317355791429937140675388300349133665'
#              '5270952304601142692468560132567137023691'
#              '2231167151676389419236542240402866331380'
#              '2447326099034502447533537836842035171250'
#              '3298812864236710263840675954706618386470'
#              '6726206802621220956394396308409166499421'
#              '2455580566739926971778789683148834896372'
#              '2136230975007644204535140061339734313395'
#              '7817532822753167159403800462161409535692'
#              '1639054296353147555888240017542436298557'
#              '8656004835718907054444374460215851541758'
#              '1980816805944769287392138652177704895540'
#              '0452088397359916079757321626267933279866'
#              '8836688757622072034633674655123263935369'
#              '0442167338253911249472180846293240627636'
#              '2069364172302388346299698267598574043616'
#              '2073352978319001743149714886811623570554'
#              '0170547183515469169233486143520189196748')
#
# ListOfNumber = []
# while OurNumber != 0:
#     OurNumber = int(OurNumber)
#     RecordOnList = (OurNumber % 100)
#     OurNumber //= 100
#     ListOfNumber.append(RecordOnList)
# ListOfNumber.reverse()
#
# def LeftRight(ListOfNumber):
#     list = []
#     LargestMulti = 0
#     while len(ListOfNumber) > 3:
#         list.append(ListOfNumber[0])
#         list.append(ListOfNumber[1])
#         list.append(ListOfNumber[2])
#         list.append(ListOfNumber[3])
#         Result = list[0] * list[1] * list[2] * list[3]
#         list.clear()
#         ListOfNumber.pop(0)
#         if Result > LargestMulti:
#             LargestMulti = Result
#     return LargestMulti
#
# print(LeftRight(ListOfNumber))

# task#11

# Последовательность треугольных чисел образуется путем сложения натуральных чисел. К примеру,
# 7-ое треугольное число равно 1 + 2 + 3 + 4 + 5 + 6 + 7 = 28. Первые десять треугольных чисел:
# 1, 3, 6, 10, 15, 21, 28, 36, 45, 55, ...
# Перечислим делители первых семи треугольных чисел:
#  1: 1
#  3: 1, 3
#  6: 1, 2, 3, 6
# 10: 1, 2, 5, 10
# 15: 1, 3, 5, 15
# 21: 1, 3, 7, 21
# 28: 1, 2, 4, 7, 14, 28
# Как мы видим, 28 - первое треугольное число, у которого более пяти делителей.
# Каково первое треугольное число, у которого более пятисот делителей?

# мой код ( считает долго 500 так и не посчитал ) максимум увидел 320

# def NumberDivider(TriangularNumber):
#     CountDivision = 0
#     for i in range(1, TriangularNumber+1):
#         if TriangularNumber % i == 0:
#             CountDivision += 1
#     print("Count div", CountDivision)
#     if CountDivision > 100:
#         return True
#     else:
#         return False
# TriangularNumber = 0
# MaxNumber = 1
# t = True
# while t is True:
#     for j in range(1, MaxNumber+1):
#         TriangularNumber += j
#     if NumberDivider(TriangularNumber) == True:
#         Answer = TriangularNumber
#         t = False
#     print("TriangularNumber", TriangularNumber)
#     MaxNumber += 1
#     TriangularNumber = 0
#
# print(Answer)

# Код из инета ( посчитал )

# import math
#
# def is_perf_sq(n):
#     x=math.ceil((math.sqrt(n)))
#     if x*x==n:
#         return True
#     else:
#         return False
#
# # Далее функция, возвращающая первое треугольное число
# def max_triangular(m):
#     x=3
#     n=2
#     while n<=m:
#         n=2
#         num=sum([c for c in range(1,x+1)])
#         for i in range(2,math.ceil(math.sqrt(num))):
#             if num%i==0:
#                 n+=2
#         if is_perf_sq(num):
#                 n-=1
#         x+=1
#     return num
#
# print(max_triangular(500))

# task#13
# Найдите первые десять цифр суммы следующих ста 50-значных чисел.

# OurNumber = ('37107287533902102798797998220837590246510135740250'
#              '46376937677490009712648124896970078050417018260538'
#              '74324986199524741059474233309513058123726617309629'
#              '91942213363574161572522430563301811072406154908250'
#              '23067588207539346171171980310421047513778063246676'
#              '89261670696623633820136378418383684178734361726757'
#              '28112879812849979408065481931592621691275889832738'
#              '44274228917432520321923589422876796487670272189318'
#              '47451445736001306439091167216856844588711603153276'
#              '70386486105843025439939619828917593665686757934951'
#              '62176457141856560629502157223196586755079324193331'
#              '64906352462741904929101432445813822663347944758178'
#              '92575867718337217661963751590579239728245598838407'
#              '58203565325359399008402633568948830189458628227828'
#              '80181199384826282014278194139940567587151170094390'
#              '35398664372827112653829987240784473053190104293586'
#              '86515506006295864861532075273371959191420517255829'
#              '71693888707715466499115593487603532921714970056938'
#              '54370070576826684624621495650076471787294438377604'
#              '53282654108756828443191190634694037855217779295145'
#              '36123272525000296071075082563815656710885258350721'
#              '45876576172410976447339110607218265236877223636045'
#              '17423706905851860660448207621209813287860733969412'
#              '81142660418086830619328460811191061556940512689692'
#              '51934325451728388641918047049293215058642563049483'
#              '62467221648435076201727918039944693004732956340691'
#              '15732444386908125794514089057706229429197107928209'
#              '55037687525678773091862540744969844508330393682126'
#              '18336384825330154686196124348767681297534375946515'
#              '80386287592878490201521685554828717201219257766954'
#              '78182833757993103614740356856449095527097864797581'
#              '16726320100436897842553539920931837441497806860984'
#              '48403098129077791799088218795327364475675590848030'
#              '87086987551392711854517078544161852424320693150332'
#              '59959406895756536782107074926966537676326235447210'
#              '69793950679652694742597709739166693763042633987085'
#              '41052684708299085211399427365734116182760315001271'
#              '65378607361501080857009149939512557028198746004375'
#              '35829035317434717326932123578154982629742552737307'
#              '94953759765105305946966067683156574377167401875275'
#              '88902802571733229619176668713819931811048770190271'
#              '25267680276078003013678680992525463401061632866526'
#              '36270218540497705585629946580636237993140746255962'
#              '24074486908231174977792365466257246923322810917141'
#              '91430288197103288597806669760892938638285025333403'
#              '34413065578016127815921815005561868836468420090470'
#              '23053081172816430487623791969842487255036638784583'
#              '11487696932154902810424020138335124462181441773470'
#              '63783299490636259666498587618221225225512486764533'
#              '67720186971698544312419572409913959008952310058822'
#              '95548255300263520781532296796249481641953868218774'
#              '76085327132285723110424803456124867697064507995236'
#              '37774242535411291684276865538926205024910326572967'
#              '23701913275725675285653248258265463092207058596522'
#              '29798860272258331913126375147341994889534765745501'
#              '18495701454879288984856827726077713721403798879715'
#              '38298203783031473527721580348144513491373226651381'
#              '34829543829199918180278916522431027392251122869539'
#              '40957953066405232632538044100059654939159879593635'
#              '29746152185502371307642255121183693803580388584903'
#              '41698116222072977186158236678424689157993532961922'
#              '62467957194401269043877107275048102390895523597457'
#              '23189706772547915061505504953922979530901129967519'
#              '86188088225875314529584099251203829009407770775672'
#              '11306739708304724483816533873502340845647058077308'
#              '82959174767140363198008187129011875491310547126581'
#              '97623331044818386269515456334926366572897563400500'
#              '42846280183517070527831839425882145521227251250327'
#              '55121603546981200581762165212827652751691296897789'
#              '32238195734329339946437501907836945765883352399886'
#              '75506164965184775180738168837861091527357929701337'
#              '62177842752192623401942399639168044983993173312731'
#              '32924185707147349566916674687634660915035914677504'
#              '99518671430235219628894890102423325116913619626622'
#              '73267460800591547471830798392868535206946944540724'
#              '76841822524674417161514036427982273348055556214818'
#              '97142617910342598647204516893989422179826088076852'
#              '87783646182799346313767754307809363333018982642090'
#              '10848802521674670883215120185883543223812876952786'
#              '71329612474782464538636993009049310363619763878039'
#              '62184073572399794223406235393808339651327408011116'
#              '66627891981488087797941876876144230030984490851411'
#              '60661826293682836764744779239180335110989069790714'
#              '85786944089552990653640447425576083659976645795096'
#              '66024396409905389607120198219976047599490197230297'
#              '64913982680032973156037120041377903785566085089252'
#              '16730939319872750275468906903707539413042652315011'
#              '94809377245048795150954100921645863754710598436791'
#              '78639167021187492431995700641917969777599028300699'
#              '15368713711936614952811305876380278410754449733078'
#              '40789923115535562561142322423255033685442488917353'
#              '44889911501440648020369068063960672322193204149535'
#              '41503128880339536053299340368006977710650566631954'
#              '81234880673210146739058568557934581403627822703280'
#              '82616570773948327592232845941706525094512325230608'
#              '22918802058777319719839450180888072429661980811197'
#              '77158542502016545090413245809786882778948721859617'
#              '72107838435069186155435662884062257473692284509516'
#              '20849603980134001723930671666823555245252804609722'
#              '53503534226472524250874054075591789781264330331690')
#
# ListOfNumber = []
# while OurNumber != 0:
#     OurNumber = int(OurNumber)
#     RecordOnList = (OurNumber % 100000000000000000000000000000000000000000000000000)
#     OurNumber //= 100000000000000000000000000000000000000000000000000
#     ListOfNumber.append(RecordOnList)
# ListOfNumber.reverse()
#
# print(sum(ListOfNumber))

# task#14
# Следующая повторяющаяся последовательность определена для множества натуральных чисел:
# n → n/2 (n - четное)
# n → 3n + 1 (n - нечетное)
# Используя описанное выше правило и начиная с 13, сгенерируется следующая последовательность:
# 13 → 40 → 20 → 10 → 5 → 16 → 8 → 4 → 2 → 1
# Получившаяся последовательность (начиная с 13 и заканчивая 1) содержит 10 элементов.
# Хотя это до сих пор и не доказано (проблема Коллатца (Collatz)),
# предполагается, что все сгенерированные таким образом последовательности оканчиваются на 1.
# Какой начальный элемент меньше миллиона генерирует самую длинную последовательность?
# Примечание: Следующие за первым элементы последовательности могут быть больше миллиона.

# def MaxCollatz(Number):
#     CountSequence = 0
#     while Number != 1:
#         if Number % 2 == 0:
#             Number /= 2
#             CountSequence += 1
#         else:
#             Number *= 3
#             Number += 1
#             CountSequence += 1
#     Result = CountSequence
#     CountSequence = 0
#     return Result
#
# def Perebor(Number):
#     MaxSequence = 0
#     while Number > 1:
#         Sequence = MaxCollatz(Number)
#         if Sequence > MaxSequence:
#             MaxSequence = Sequence
#             Answer = Number
#             Number -= 1
#         else:
#             Number -= 1
#     return Answer
#
# print("Enter your number: ")
# Inputer = int (input())
# print(Perebor(Inputer))

# task#15

# task#16
# 215 = 32768, сумма цифр этого числа равна 3 + 2 + 7 + 6 + 8 = 26.
# Какова сумма цифр числа 21000?

# Number = 2**1000
# list = []
# while Number != 0:
#     list.append(Number % 10)
#     Number //= 10
# list.reverse()
#
# print(sum(list))

# task#17
# Если записать числа от 1 до 5 английскими словами (one, two, three, four, five), то используется всего 3 + 3 + 5 + 4 + 4 = 19 букв.
# Сколько букв понадобится для записи всех чисел от 1 до 1000 (one thousand) включительно?
# Примечание: Не считайте пробелы и дефисы. Например, число 342 (three hundred and forty-two)
# состоит из 23 букв, число 115 (one hundred and fifteen) - из 20 букв.
# Использование "and" при записи чисел соответствует правилам британского английского.

# Мой код

# Numbers = {
#     1: 'one',
#     2: 'two',
#     3: 'three',
#     4: 'four',
#     5: 'five',
#     6: 'six',
#     7: 'seven',
#     8: 'eight',
#     9: 'nine',
#     10: 'ten',
#     11: 'eleven',
#     12: 'twelve',
#     13: 'thirteen',
#     14: 'fourteen',
#     15: 'fifteen',
#     16: 'sixteen',
#     17: 'seventeen',
#     18: 'eighteen',
#     19: 'nineteen',
#     20: 'twenty',
#     30: 'thirty',
#     40: 'forty',
#     50: 'fifty',
#     60: 'sixty',
#     70: 'seventy',
#     80: 'eighty',
#     90: 'ninety',
#     100: 'hundred',
#     1000: 'thousand'}
#
#
# def SizeSymbolNumber(Number):
#     CountSymbolNumber = 0
#     while Number != 0:
#         CountSymbolNumber += 1
#         Number //= 10
#     return CountSymbolNumber
#
# def DefinitionInNumber(Number, ListNumbers, CountSymbolNumber, Numbers):
#     ListNumbers = []
#     if CountSymbolNumber == 1:
#         ListNumbers.append(len(Numbers[Number]))
#         return ListNumbers
#     elif CountSymbolNumber == 2:
#         if Number % 10 == 0:
#             ListNumbers.append(len(Numbers[Number]))
#             return ListNumbers
#         else:
#             if Number >= 20:
#                 FirstSymbol = Number // 10
#                 ListNumbers.append(len(Numbers[FirstSymbol*10]))
#                 SecondSymbol = Number % 10
#                 ListNumbers.append(len(Numbers[SecondSymbol]))
#             else:
#                 ListNumbers.append(len(Numbers[Number]))
#             return ListNumbers
#     elif CountSymbolNumber == 3:
#         if Number % 100 == 0:
#             FirstSymbol = Number // 100
#             ListNumbers.append(len(Numbers[FirstSymbol]))
#             ListNumbers.append(len(Numbers[100]))
#             return ListNumbers
#         else:
#             FirstSymbol = Number // 100
#             ListNumbers.append(len(Numbers[FirstSymbol]))
#             ListNumbers.append(len(Numbers[100]))
#             ListNumbers.append(len("and"))
#             ThirdSymbol = (Number % 100 and Number % 10)
#             SecondSymbol = Number % 100
#             SecondSymbol -= ThirdSymbol
#             if (Number % 100 > 10 and Number % 100 < 20):
#                 ListNumbers.append(len(Numbers[Number % 100]))
#                 return ListNumbers
#             if SecondSymbol != 0:
#                 if ThirdSymbol != 0:
#                     ListNumbers.append(len(Numbers[SecondSymbol]))
#                     ListNumbers.append(len(Numbers[ThirdSymbol]))
#                 elif ThirdSymbol == 0:
#                     ListNumbers.append(len(Numbers[SecondSymbol]))
#             elif SecondSymbol == 0:
#                 ListNumbers.append(len(Numbers[ThirdSymbol]))
#             return ListNumbers
#     elif CountSymbolNumber == 4:
#         ListNumbers.append(len("one"))
#         ListNumbers.append(len(Numbers[1000]))
#         return ListNumbers
#
# list = []
# ListNumbers = []
# for Number in range(1, 1000+1):
#     CountSymbolNumber = SizeSymbolNumber(Number)
#     ListNumbers = DefinitionInNumber(Number, ListNumbers, CountSymbolNumber, Numbers)
#     list.append(sum((ListNumbers)))
#
# print(sum(list))

# Код из инета

# nums={
#     1:'one',
#     2:'two',
#     3:'three',
#     4:'four',
#     5:'five',
#     6:'six',
#     7:'seven',
#     8:'eight',
#     9:'nine',
#     10:'ten',
#     11:'eleven',
#     12:'twelve',
#     13:'thirteen',
#     15:'fifteen',
#     18:'eighteen',
#     20:'twenty',
#     30:'thirty',
#     40:'forty',
#     50:'fifty',
#     60:'sixty',
#     70:'seventy',
#     80:'eighty',
#     90:'ninety',
#     }
#
# def gen_num(n,dic):
#     tmp=''
#     t=str(n)
#     if dic.get(n,False):
#         return dic[n]
#     elif n<20:
#         return dic[int(t[1])]+'teen'
#     elif n<100:
#         tmp=n-int(t[1])
#         return dic[tmp]+dic[int(t[1])]
#     elif n<1000:
#         if t[1:3]=='00':
#             return dic[int(t[0])]+'hundred'
#         else:
#             return dic[int(t[0])]+'hundredand'+gen_num(int(t[1:3]),nums)
#     elif n==1000:
#         return 'onethousand'
#
# s = 0
# for i in range(99, 1001):
#     s += len(gen_num(i, nums))
#     print(gen_num(i, nums))
# print(s)

# task#20
# n! означает n × (n − 1) × ... × 3 × 2 × 1
# Например, 10! = 10 × 9 × ... × 3 × 2 × 1 = 3628800,
# и сумма цифр в числе 10! равна 3 + 6 + 2 + 8 + 8 + 0 + 0 = 27.
# Найдите сумму цифр в числе 100!.

# def FactorialCalculation(Number):
#     FactorialNumber = 1
#     while Number != 1:
#         FactorialNumber *= Number
#         Number -= 1
#     return SumNumeralsNumber(FactorialNumber)
#
# def SumNumeralsNumber(FactorialNumber):
#     StrNum = str(FactorialNumber)
#     Summ = 0
#     for i in StrNum:
#         Summ += int (i)
#     return Summ
#
# print(FactorialCalculation(100))
